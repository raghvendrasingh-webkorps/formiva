export interface FormivaConfig<T> {
  initialValues: T
  validate?: (values: T) => Partial<Record<keyof T, string>>
  onSubmit?: (values:T) => void
}

export interface FormivaState<T> {
  values : T
...
export function formiva<T extends Object> (config: FormivaConfig<T>) {

  let state  : FormivaState<T> = {
  ...
  type Listener<T> = (state: FormivaState<T>)=> void;
